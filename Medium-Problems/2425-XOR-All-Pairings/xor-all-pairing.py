...
def xorAllNums(nums1: list[int], nums2: list[int]) -> int:
    # Calculate the lengths of the two input lists for simplicity
    n1 = len(nums1)
    n2 = len(nums2)
    ...
    # Conditional to check if the second list has an odd length
    if n2 % 2 == 1:
    ...
    print(f"Result with XOR Nums1: {result}")

    # Conditional to check if the first list has an odd length
    if n1 % 2 == 1:
    ...
    print(f"Result with XOR Nums2: {result}")

    # Return the final result based on the conditions and the cumulative XOR calculations
